<template>
    <div class="card card-primary">
        ...
        <div class="card-body">
                ...
                <div class="media">
                    <div class="media-body">
                        <p class="mt-0"><span class="h5">{{ comment.user.name }}</span> 
                            <span v-if="comment.user.type == 'agent'" class="mx-2 badge badge-success">Agent</span> 
                            <span v-else class="mx-2 badge badge-primary">User</span> 
                            Says: 
            ...
            <span class="badge" v-if="ticket.comments.length == 0">No Replies Are Available</span>
...
export default {
    ...
    methods:{
        createComment(){
            this.$inertia.post(route('tickets.create_comment', this.ticket.id), this.form,{
                preserveScroll: true,
            });
            this.form.comment = null
        },
        deleteComment(id){
            this.$inertia.delete(route('tickets.delete_comment', id),{
                preserveScroll: true,
            });
        },
    }
